...
class Test(TestCase):
    ...
    def test_detect_release_type(self):
        ...
        # Minor Release
        self.assertEqual(
            first=detect_release_type(commits=[feat_commit, fix_commit, ci_commit]),
            second="minor",
        )

        # Patch Release
        self.assertEqual(
            first=detect_release_type(commits=[fix_commit, ci_commit]), second="patch"
        )
    ...
    def test_get_commits(self):
        ...
        with TemporaryDirectory() as tmpdirname:
            ...
            # Test Commits
            self.assertEqual(first=commits, second=["feat: initial commit"])

            # Test Release Type
            self.assertEqual(first=release_type, second="minor")

            # Test Semantic Versioning
            self.assertEqual(
                first=bump_version(version=my_semver, release_type=release_type),
